...
public class SelfTrap extends ObsidianPlacerModule
{
    ...
    public Setting<Boolean> headExtend = bool("Extend-Head", false);
    ...
    public SelfTrap()
    {
        super("SelfTrap", "Surrounds feet with obsidian", Category.COMBAT,true,false,false, 950);
        INSTANCE = this;
    }
    ...
    public List<BlockPos> getSurround(BlockPos playerPos, PlayerEntity player)
    {
        ...
        if (mineExtend.getValue())
        {
            for (BlockPos surroundPos : new ArrayList<>(surroundBlocks))
            {
                ...
                if (!headExtend.getValue() && secondLayer)
                {
                    continue;
                }
                ...
                if (secondLayer && MoneyStar.interactionManager.getInteractDirectionInternal(surroundPos,
                        strictDirection.getValue()) == null)
                {
                    continue;
                }
                ...
                for (Direction direction : Direction.values())
                {
                    ...
                    if (playerBlocks.contains(blockerPos)
                            || AutoMine.getInstance().getMiningBlock() == blockerPos
                    )
                        // Dont want to help our opponent surround
                    {
                        continue;
                    }
