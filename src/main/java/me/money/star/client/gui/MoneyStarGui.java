...
import me.money.star.client.gui.items.buttons.ModuleButton;
...
public class MoneyStarGui extends Screen {
    private static MoneyStarGui oyveyGui;
    private static MoneyStarGui INSTANCE;

    static {
        INSTANCE = new MoneyStarGui();
    }

    private final ArrayList<Component> components = new ArrayList();

    public MoneyStarGui() {
        super(Text.literal("Money-Star"));
        setInstance();
        load();
    }

    public static MoneyStarGui getInstance() {
        if (INSTANCE == null) {
            INSTANCE = new MoneyStarGui();
        }
        return INSTANCE;
    ...
    public static MoneyStarGui getClickGui() {
        return MoneyStarGui.getInstance();
    }
    ...
    private void load() {
        int x = -84;
        for (final Module.Category category : MoneyStar.moduleManager.getCategories()) {
            this.components.add(new Component(category.getName(), x += 102, 4, true) {

                @Override
                public void setupItems() {
                    counter1 = new int[]{1};
                    MoneyStar.moduleManager.getModulesByCategory(category).forEach(module -> {
                        if (!module.hidden) {
        ...
        this.components.forEach(components -> components.getItems().sort(Comparator.comparing(System::getName)));
    }
