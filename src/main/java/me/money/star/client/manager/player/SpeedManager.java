...
public class SpeedManager
    ...
    public float lastJumpInfoTimeRemaining() {
        return (float) (java.lang.System.currentTimeMillis() - this.jumpInfoStartTime) / 1000.0f;
    }

    public void updateValues() {
        double distTraveledLastTickX = Util.mc.player.getX() - Util.mc.player.prevX;
        double distTraveledLastTickZ = Util.mc.player.getZ() - Util.mc.player.prevZ;
        this.speedometerCurrentSpeed = distTraveledLastTickX * distTraveledLastTickX + distTraveledLastTickZ * distTraveledLastTickZ;
        if (didJumpThisTick && (!Util.mc.player.isOnGround() || isJumping)) {
            if (didJumpThisTick && !this.didJumpLastTick) {
                ...
                this.jumpInfoStartTime = java.lang.System.currentTimeMillis();
    ...
    public void updatePlayers() {
        for (PlayerEntity player : Util.mc.world.getPlayers()) {
            if (!(Util.mc.player.distanceTo(player) < distancer))
                continue;
